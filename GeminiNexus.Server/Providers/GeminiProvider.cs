...
public sealed partial class GeminiProvider(IHttpClientFactory factory,ServerOptions options) : IChatProvider,IModelCatalog,ITokenCounter,IProviderExplorer
{
    ...
    public async Task<int> Count(string model,string contents,CancellationToken ct)
    {
        ...
        var body=await ReadBounded(response.Content,options.MaxEventBytes,ct);if(!response.IsSuccessStatusCode)throw new WorkspaceException((int)response.StatusCode,"ספירת הטוקנים נכשלה: "+body);
    ...
    public async Task<string> Prepare(StoredRunRequest stored,CancellationToken ct)
    {
        ...
        while(true)
        {
            using var req=Request(HttpMethod.Post,$"models/{stored.Request.Model}:countTokens",new JsonObject{["generateContentRequest"]=new JsonObject{["model"]="models/"+stored.Request.Model,["contents"]=contents.DeepClone(),["systemInstruction"]=config["systemInstruction"]!.DeepClone(),["tools"]=config["tools"]?.DeepClone()}}.ToJsonString());
            ...
            if(!response.IsSuccessStatusCode)throw new WorkspaceException((int)response.StatusCode,"לא ניתן לאמת את תקציב ההקשר: "+body);
    ...
    public async IAsyncEnumerable<ProviderChunk> Stream(string model,string payload,[EnumeratorCancellation] CancellationToken ct)
    {
        ...
        using var response=await factory.CreateClient("Gemini").SendAsync(request,HttpCompletionOption.ResponseHeadersRead,ct);
        if(!response.IsSuccessStatusCode)throw new WorkspaceException((int)response.StatusCode,await ReadBounded(response.Content,options.MaxEventBytes,ct));
        await using var stream=await response.Content.ReadAsStreamAsync(ct);
        await foreach(var raw in SseReader.Read(stream,options.MaxEventBytes,ct))yield return Parse(raw);
    }
    ...
    public async Task<PlaygroundResponse> Execute(PlaygroundRequest request,CancellationToken ct)
    {
        ...
        return new((int)response.StatusCode,response.Content.Headers.ContentType?.ToString()??"application/json",await ReadBounded(response.Content,options.MaxTraceBytes,ct));
    }
}
