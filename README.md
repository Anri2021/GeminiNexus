...
- אימות Cookie, בדיקת בעלות בכל גישה לשיחה ולריצה, הגנת בקשות משנות מצב והגבלת קצב נפרדת להתחברות. סודות נקראים מסביבת התהליך או מקובץ חיצוני.
...
- הזרמת SSE עם מספרי רצף וחיבור מחדש, חלונית אירועים גולמיים ונתוני שימוש מדווחים. סיום הריצה, אירוע הסיום והתשובה נשמרים בעסקה אחת.
- עימוד הודעות ושיחות, באפר הודעות מוגבל, גלילה וירטואלית, עוגן גלילה וגלילה לתחתית בהתאם למיקום המשתמש.
...
יצירת משתמש נוסף נעשית דרך `POST /api/admin/users` עם `{name,password}` מתוך סשן מנהל ועם כותרת `X-Nexus-Request: 1`. סיסמת המנהל מהסביבה מעודכנת בעת אתחול; ביטול סשנים קיימים דורש ניהול מפתחות Data Protection. אין כרגע מסך ניהול משתמשים או מנגנון איפוס סיסמה בדוא״ל.
...
| גלילה וזיכרון | בדיקת עומס עם מדיה והודעות בגבהים משתנים; מדיניות הוצאת עמודים דו־כיוונית ו־PrefetchPages מלא; הודעה בודדת ומדיה עשויות לחרוג מבאפר התצוגה |
...
| תפעול משתמשים | ניהול חשבונות, איפוס סיסמה וביטול סשנים פרטני; כללי שימור שיחות וקבצים, ניטור תפעולי ומבחני התאוששות ועומס |
