...
לאחר מכן פותחים `http://127.0.0.1:5000/` ונכנסים בשם המשתמש `admin` ובסיסמה שנשמרה באמצעות הסקריפט. השרת מארח את ממשק ה־WASM ואת ה־API מאותה כתובת, ולכן Cookie ההתחברות נשאר מוגן מפני שימוש בין מקורות. אין סיסמת ברירת מחדל במאגר.
