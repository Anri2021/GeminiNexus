# Gemini Nexus

סביבת שיחות בעברית, במראה בהשראת Gemini, עם API עצמאי וממשק Blazor משותף לדפדפן ול־MAUI.

## 1. מימוש נוכחי

- חמישה פרויקטי מוצר: Shared, UI, Server, Client.Wasm, Client.Maui; יעדים .NET 11 / C# 15. גרסת SDK נעולה ב־global.json.
- אימות Cookie עם סשנים ניתנים לביטול, ניהול משתמשים והחלפת סיסמה, בדיקת בעלות בכל גישה לשיחה ולריצה, הגנת בקשות משנות מצב והגבלת קצב נפרדת להתחברות. סודות נקראים מסביבת התהליך או מקובץ חיצוני.
- שיחות שמורות, שני צדדי השיחה, חיפוש כותרות, נעיצה, ארכוב, עריכה באמצעות הסתעפות והקשר שנשלח בפועל למודל.
- תור מתמשך במסד הנתונים עם Outbox, תזמון Channels מוגבל, ביטול, מכסות מקביליות, פעימות חיות ושחזור תשובה חלקית לאחר אובדן תהליך. ריצה שנקטעה אינה נוצרת מחדש אצל הספק אוטומטית.
- הזרמת SSE עם מספרי רצף וחיבור מחדש, חלונית אירועים גולמיים ונתוני שימוש מדווחים שנשמרים גם לאחר פינוי אירועי הדיבאג, כולל זמן לתוכן ראשון, זמן בתור וזמן עיבוד. סיום הריצה, אירוע הסיום והתשובה נשמרים בעסקה אחת.
- עימוד הודעות דו־כיווני ושיחות, טעינה מוקדמת לפי כיוון הגלילה, באפר הודעות מוגבל, גלילה וירטואלית, עוגן גלילה וגלילה לתחתית בהתאם למיקום המשתמש.
- קבצים בקלט כחלקי inlineData, הצגת תמונות ואודיו נתמכים, Markdown בסיסי מקודד נגד HTML זדוני והעתקת קוד.
- תוספי פרומפט דטרמיניסטיים: prefix, suffix, system, replace. הגדרות מסונכרנות, וביצוע בשרת או בלקוח.
- מתאם generateContent, קטלוג מודלים וספירת טוקנים רשמית הכוללת הוראות מערכת וכלים. מעבדת API למנהל עבור בקשות JSON חד־פעמיות.

## 2. בנייה ובדיקות

יש להתקין את ה־SDK המדויק שב־global.json. נדרשת גישה ל־NuGet לבנייה מלאה.

PowerShell:

```powershell
./scripts/Publish.ps1
python ./tests/integration.py
python ./tests/source_checks.py
```

Bash:

```bash
bash scripts/publish.sh
python3 tests/integration.py
python3 tests/source_checks.py
```

הסקריפטים מפרסמים בנפרד את השרת ואת Client.Wasm. בדיקות האינטגרציה מפעילות שרת אמיתי מול ספק בדיקה מקומי, עם נתונים וסודות זמניים, ללא קריאות Google בתשלום. בדיקות המקור מקמפלות את החוזים, רכיבי Razor ומתאם הספק ומריצות בדיקות SSE, סריאליזציה, השחרה ו־Markdown; הן אינן תחליף לבדיקות מסד הנתונים או לבניית WASM/MAUI.

תהליך GitHub Actions בקובץ `.github/workflows/verify.yml` כולל פרסום השרת והממשק, בדיקות אינטגרציה, בדיקות מקור ובניית MAUI ל־Windows. אין להתייחס לתהליך שעוד לא רץ כאל בדיקה שעברה.

## 3. פריסה דרך Caddy

1. לבטל את מפתחות Gemini שהיו בגרסאות הישנות וליצור מפתחות חדשים. הסרתם מקובצי המקור אינה מבטלת את המפתחות או מוחקת היסטוריית Git.
2. להגדיר `Auth__AdminPasswordFile` ו־`Gemini__ApiKeyFile` לקבצים חיצוניים, או את `Auth__AdminPassword` ו־`Gemini__ApiKey` בסביבת התהליך. סיסמת המנהל חייבת להכיל לפחות 16 תווים. אין לשמור ערכים אלה במאגר. אין לשתף אותם בצ׳אט.
3. להעתיק את `artifacts/publish/server` אל `/srv/gemininexus/server`, ואת `artifacts/publish/client` אל `/srv/gemininexus/client`.
4. להתאים ב־`deploy/server.env.example` את `AllowedHosts` ואת נתיבי הסודות, וליצור קובץ `/etc/gemininexus/server.env` עם הרשאות שירות בלבד. ב־`deploy/gemininexus.service` להתאים את הנתיב ל־dotnet ואת משתמש השירות. ליצור משתמש שירות ייעודי לפני התקנה.
5. ב־`deploy/Caddyfile` להגדיר את `NEXUS_DOMAIN` בסביבת Caddy לדומיין האמיתי. התצורה מניחה ש־Caddy והשרת על אותו מארח: API בפורט פנימי 5000 וקובצי WASM מוגשים מהדיסק. יש להתאים כתובת upstream ורשימת `Proxy__KnownProxies__0` אם Caddy נמצא במארח אחר; אין לסמוך על כל Proxy.
6. לבדוק תצורת Caddy באמצעות `caddy validate --config /path/to/Caddyfile`, להפעיל את השירותים, ואז לבדוק `/health/ready`, התחברות, שיחה ורענון דפדפן דרך הדומיין וה־HTTPS האמיתיים.
7. לגבות את מסד הנתונים ואת `Auth:KeyPath` באחסון מוגן. ב־SQLite יש להשתמש ב־Online Backup או לעצור את השירות ולגבות את המסד עם קובצי WAL/SHM. לפני פתיחת השירות למשתמשים יש לבצע שחזור ניסיון.

השרת דורש HTTPS בפרודקשן. `Auth:AllowInsecureLocal=true` מותר רק ב־Development או Testing. MAUI מבקש כתובת HTTPS של השרת ומשתמש באותו Workspace מה־RCL, עם CookieContainer נפרד מהדפדפן. אין עקיפת אישורי TLS.

אפשר ליצור משתמשים ולנתק את כל המכשירים שלהם בחלונית ״חשבון ואבטחה״. יצירת משתמש נוסף זמינה גם דרך `POST /api/admin/users` עם `{name,password}` מתוך סשן מנהל ועם כותרת `X-Nexus-Request: 1`. סיסמת המנהל מהסביבה מעודכנת בעת אתחול; שינוי שלה מבטל את סשני המנהל, ואתחול ללא שינוי משמר אותם. משתמש רגיל יכול לשנות סיסמה בחלונית החשבון באמצעות הסיסמה הקיימת; הפעולה מבטלת את כל החיבורים הקודמים. סשן תקף לשבעה ימים לכל היותר, ונבדק מול המסד בכל בקשה. אין איפוס סיסמה בדוא״ל.

## 4. גבולות מוכנות והשלמות נדרשות

המימוש אינו מסומן כפרודקשן מלא עד שכל הבדיקות וההשלמות הרלוונטיות הסתיימו:

| תחום | מה עדיין נדרש |
|---|---|
| אימות סביבת היעד | בנייה מלאה עם חבילות משוחזרות, בדיקות בדפדפן על WASM אמיתי, MAUI במכשירי היעד, בדיקת Gemini אמיתי, דומיין ו־TLS, גיבוי ושחזור |
| גלילה וזיכרון | בדיקת עומס עם מדיה והודעות בגבהים משתנים; הודעה בודדת ומדיה עשויות לחרוג מבאפר התצוגה |
| PostgreSQL וריבוי שרתים | המתאם קיים אך דורש בדיקות מסד נפרדות, הגירות סכימה עם גרסאות ובדיקות תחרות בין תהליכים. SQLite מיועד למופע שרת יחיד עם קובץ מקומי |
| API מתקדם | ממשקי מוצר מלאים ל־Interactions, Live, Batch, Embeddings, קבצים מרוחקים, cache, וידאו ומדיה; מעבדת JSON אינה מממשת מסכים אלה |
| כלים ותוספים | מנוע חבילות מבודד עם Manifest והרשאות, התקנה וגרסאות, תוצאות functionCall, וסביבות WASM/Native. כרגע תוספי פרומפט בלבד; functionCall מסתיים ב־requires_action |
| AOT וביצועים | פרסום NativeAOT בפועל ובדיקת trim של כל חבילה; מדדי הקצאות, עומס וזמן תגובה. השימוש ב־Source Generators אינו הוכחה לאפס הקצאות |
| ספריות וחישוב מקומי | שילוב תפעולי של Refit, Polly, Mapperly, gRPC, Dapper.AOT/EF compiled queries לפי תפקיד, ויחידות Native/GPU/Rust/C++/bflat/NoGC. אין להציג חבילת NuGet שאינה בשימוש כמימוש של דרישה |
| תפעול משתמשים | איפוס סיסמה בדוא״ל; כללי שימור שיחות וקבצים, ניטור תפעולי ומבחני התאוששות ועומס |

היסטוריית הודעות אינה נמחקת כשמפנים את באפר הלקוח. אירועי הדיבאג כפופים ל־`Limits:TraceRetentionDays`; זרם אירועים שכבר נמחק נסגר באופן תקין. השדות הגולמיים משמרים מידע שהספק מחזיר, לאחר השחרת שדות סוד מוכרים; אין גישה למידע שהספק לא חשף.

מקורות: [API של Google](https://ai.google.dev/api/generate-content), [תיעוד Caddy](https://caddyserver.com/docs/caddyfile/directives/reverse_proxy).

## 5. אימות השינויים בענף זה

- עברו 11 בדיקות אינטגרציה מקומיות מול ספק HTTP מדומה: בעלות, CSRF, שמירה והקשר, שחזור אחרי הפסקת שרת, מקביליות וביטול, זהות בקשות, עימוד דו־כיווני, מדדי שימוש מתמשכים, משתמשים וביטול חיבורים.
- עברו 10 בדיקות מקור: קימפול ורינדור רכיבי Razor, פענוח SSE מפוצל בעברית, גבולות גודל, JSON ללא Reflection, השחרת סודות ותוכן Markdown.
- שרת הבדיקה קומפל מהמקור הנוכחי ב־.NET 11, מול DLL של התלויות מהבנייה המקומית הקודמת; רישום סוגי ה־Endpoints בבדיקת האופליין היה מפורש. זה אינו אימות של שחזור NuGet או הרצת FastEndpoints.Generator בבנייה המלאה.
- שחזור NuGet בסביבת העבודה נחסם ברשת. לפיכך בניית WASM מלאה, פרסום, NativeAOT ו־MAUI לא אושרו. אין Chromium מקומי, ולכן בדיקת הדפדפן לא הורצה. תהליך CI מגדיר בדיקות אלה, ויש לבדוק את תוצאתו בפועל.
- לא בוצעו פריסה לשרת היעד, שינוי DNS, החלפת מפתחות אצל Google או בדיקה עם Gemini חי. מפתחות שנחשפו בעבר עדיין דורשים ביטול אצל הספק.
