...
client_output="$(mktemp -d)"
trap 'rm -rf "$client_output"' EXIT
...
dotnet publish GeminiNexus.Client.Wasm -c Release -o "$client_output" --disable-build-servers
mkdir -p "$output/server/wwwroot"
cp -R "$client_output/wwwroot/." "$output/server/wwwroot/"
